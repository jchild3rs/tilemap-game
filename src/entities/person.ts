...
export class PersonEntity extends Effect.Service<PersonEntity>()(
	"entities/Person",
	{
		effect: Effect.gen(function* () {
			...
			const create = (position: PositionLiteral, status: CombatStatus) =>
				Effect.gen(function* () {
					...
					// const graphic = new PIXI.Graphics({ label: "person" })
					// 	.circle(
					// 		config.CELL_SIZE / 2,
					// 		config.CELL_SIZE / 2,
					// 		config.CELL_SIZE / 2,
					...
					// 	});
					//
					// const container = new PIXI.Container({ label: "person container" });
					...
					entity.addComponent(
						Components.Movement({
							speed: config.person.baseMovementSpeed,
							path: [],
							currentPathIndex: 0,
							direction: MovementDirection.Down,
							isMoving: false,
							isWandering: status === "friendly",
							pendingPath: [],
							currentStepIndex: 0,
						}),
					);
					...
					entity.addComponent(
						Components.Draftable({
							isDrafted: status === "hostile",
							fireAtWill: status === "hostile",
						}),
					);
					entity.addComponent(
						Components.Weapon({
					...
					//
					// container.addChild(
					// 	new PIXI.Graphics({ label: "person" })
					// 		.circle(
					// 			config.CELL_SIZE / 2,
					// 			config.CELL_SIZE / 2,
					// 			config.CELL_SIZE / 2,
