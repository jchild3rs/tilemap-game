		...
		const makeBody = (status: CombatStatus) => {
			...
			const shirtMask = torsoContainer.addChild(
				new PIXI.Graphics({ label: "Shirt Mask" })
					.ellipse(config.CELL_SIZE / 2, config.CELL_SIZE / 2, config.CELL_SIZE / 2, config.CELL_SIZE / 4)
					.fill("#000000")
					.stroke({ width: 2, color: 0x000000 }),
			);
			...
			const legsContainer = container.addChild(new PIXI.Container({ label: "Legs" }));
			legsContainer.position.set(0, 0);

			const pantsMask = legsContainer.addChild(
				new PIXI.Graphics({ label: "Pants Mask" })
					.rect(0, config.CELL_SIZE * 0.7, config.CELL_SIZE, config.CELL_SIZE / 2)
					.fill("transparent"),
			);
			...
			const headContainer = container.addChild(new PIXI.Container({ label: "Head" }));
			headContainer.addChild(
				new PIXI.Graphics({ label: "Head" })
					.circle(config.CELL_SIZE / 2, config.CELL_SIZE / 3.5, config.CELL_SIZE / 4)
					.fill(skinColor)
					.stroke({ width: 2, color: 0x000000 }),
			);
			...
			const leftEye = headContainer.addChild(
				new PIXI.Graphics({ label: "Left Eye" })
					.circle(config.CELL_SIZE / 2 - config.CELL_SIZE / 16, config.CELL_SIZE / 4, config.CELL_SIZE / 24)
					.fill(0x000000),
			);
			...
			const gunRadiusGraphic = container.addChild(
				new PIXI.Graphics({ alpha: 0.25, label: "Gun Radius" })
					.circle(config.CELL_SIZE / 2, config.CELL_SIZE / 2, config.CELL_SIZE * BASE_GUN_RANGE)
