...
export class Grid {
	 ...
	 * @param costs - A matrix representing the cost of walking the node.
	...
	private _buildNodes(
		...
		if (
			costs !== undefined &&
			(costs.length !== height || costs[0].length !== width)
		) {
	 ...
	 * @param x - The x coordinate of the node.
	 * @param y - The y coordinate of the node.
	 * @returns Cost to walk node.
	 */
	getCostAt(x: number, y: number): number | false {
	 ...
	 * @param x - The x coordinate of the node.
	 * @param y - The y coordinate of the node.
	 * @param cost - Cost to walk the node.
	 */
